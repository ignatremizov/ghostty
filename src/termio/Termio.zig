...
pub fn deinit(self: *Termio) void {
    ...
    if (self.initial_scrollback_file) |file| file.close();
}
...
pub fn replayScrollback(self: *Termio, file: std.fs.File) !void {
    ...
    replay: {
        var replay_stream: stream_terminal.Stream = .initAlloc(self.alloc, .{
            ...
            .default_cursor_blink = self.config.cursor_blink orelse true,
        ...
        while (true) {
            const n = file.read(buf[0..]) catch |err| {
                replay_error = err;
                break :replay;
            };
            if (n == 0) break;
...
fn validateReplayScrollbackFile(file: std.fs.File, max_bytes: u64) !void {
...
test "saved scrollback replay validates regular bounded files" {
    ...
    {
        const file = try tmp.dir.createFile("small.vt", .{ .read = true });
        defer file.close();
        try file.writeAll("hello\n");
        try validateReplayScrollbackFile(file, 1024);
    }
    {
        const file = try tmp.dir.createFile("large.vt", .{ .read = true });
        defer file.close();
        try file.setEndPos(1025);
