...
pub fn workspaceControlListAlloc(
    ...
    for (0..@intCast(n)) |i| {
        ...
        var result: ?WorkspaceControlWorkspace = .{
            .workspace_id = runtime.workspace.workspace_id,
...
pub fn workspaceControlListSessionsAlloc(
    ...
    for (resolved_workspace.runtime.sessions.items) |session| {
        ...
        var owned_title: ?[]u8 = try alloc.dupe(u8, title);
        errdefer if (owned_title) |value| alloc.free(value);
        var owned_cwd: ?[]u8 = try alloc.dupe(u8, session.cwd);
        errdefer if (owned_cwd) |value| alloc.free(value);
        var result: ?WorkspaceControlSession = .{
